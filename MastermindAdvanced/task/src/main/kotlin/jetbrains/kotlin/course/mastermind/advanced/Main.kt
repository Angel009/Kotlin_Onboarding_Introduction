...
fun isCorrectInput(userInput: String, wordLength: Int, alphabet: String): Boolean{
    ...
    val filteredWord: String = userInput.filter { it in alphabet }
    if (filteredWord.length < userInput.length){
...
fun safeUserInput(wordLength: Int, alphabet: String): String{
    var guess: String
    do {
        println("Please input your guess. It should be the length $wordLength, and each symbol should be from the alphabet $alphabet.")
        guess = safeReadLine()
        val valid: Boolean = isCorrectInput(guess, wordLength, alphabet)
